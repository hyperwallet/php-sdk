...
use Services\Jose\Exception\VerificationFailed;

class URLSafeBase64 extends JOSE_JWT {
    ...
    function sign($private_key_or_secret, $algorithm = 'HS256') {
        ...
        if (!$this->signature) {
            throw new Exception('Signing failed because of unknown reason');
        }
        return $this;
    }
    ...
    function verify($public_key_or_secret, $alg = null) {
        if ($this->_verify($public_key_or_secret, $alg)) {
            ...
            throw new VerificationFailed('Signature verification failed');
        }
    }
